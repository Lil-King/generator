...
import com.ebiz.generator.util.FileHelper;
import com.ebiz.generator.util.IOHelper;
...
public class Generator {
	...
	public void generateByModelProvider(IGeneratorModelProvider modelProvider) throws Exception {
		if (templateRootDirs.size() == 0)
			throw new IllegalStateException("'templateRootDirs' is empty");
	...
	private void generateByModelProvider(File templateRootDir, IGeneratorModelProvider modelProvider) throws Exception {
		...
		for (int i = 0; i < templateFiles.size(); i++) {
			...
			if (templateRelativePath.trim().equals(""))
				continue;
			if (templateFile.getName().toLowerCase().endsWith(".exclude")) {
				System.out.println("[skip]\t\t endsWith '.exclude' template:" + templateRelativePath);
				continue;
			}
			...
			try {
				generateNewFileOrInsertIntoFile(modelProvider, newFreeMarkerConfiguration(), templateRelativePath,
						outputFilePath);
			} catch (Exception e) {
				throw new RuntimeException("generate '" + modelProvider.getDisaplyText() + "' oucur error,template is:"
	...
	private void generateNewFileOrInsertIntoFile(IGeneratorModelProvider modelProvider, Configuration config,
